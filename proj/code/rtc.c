...
unsigned long read_bcd(unsigned long i) {

	i = (((i) >> 4) * 10) + (i & 0x0F);
	return i;
}

unsigned char write_bcd(unsigned char i) {
...
void rtc_write(unsigned long reg, unsigned long data) {
...
int rtc_subscribe(void) {
	...
	sys_irqsetpolicy(RTC_IRQ, IRQ_REENABLE, &rtc_hook);
